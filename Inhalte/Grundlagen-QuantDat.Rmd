...
***Nein***, die gleichen Temperaturen entsprechen in Fahrenheit 50°F und 68°F -- keine Verdoppelung. 0° Celsius ist kein absoluter Nullpunkt. 
...
mosaicplot(apply(UCBAdmissions, c(1, 2), sum),
           main = "Zulassungen an der UC Berkeley", col=trellis.par.get("superpose.polygon")$col, cex.axis = 1)
```
...
```{r plot-sample-procedures, echo=FALSE, out.width = "80%", fig.align="center"}
data(iris)
set.seed(42)
sam <- rep(1,150)
...
p1 <- gf_jitter(Petal.Width ~ Petal.Length, 
         color = ~ Species,
         data = iris,
         # alpha = ~ sam,
         shape = ~Species,
  ...
  gf_labs(title = "Vollerhebung",
          subtitle = "Alle 150 Objekte der Grundgesamtheit wurden erhoben",
       x = "X",
       y = "Y") +
...
set.seed(42)
iris_sample <- sample_n(iris, size = 30)
...
p2 <- ggplot(iris) +
  ...
  geom_jitter(color = "grey80",
             size = 4,
             aes(shape = Species)) +
  geom_jitter(data = iris_sample,
             aes(color =  Species,
                 shape = Species),
             size = 4) +
  theme(legend.position = "none") +
  labs(title = "Einfache Zufallsstichprobe",
       subtitle = "30 zufällig ausgewählte Beobachtungen",
       x = "X",
       y = "Y") +
  theme(axis.ticks = element_blank(),
        axis.text = element_blank(),
...
p3 <- gf_jitter(Petal.Width ~ Petal.Length, 
         color = "grey80",
         data = iris,
         #size = ~ sam,
         shape = ~Species,
         size = 4) %>% 
  gf_jitter(Petal.Width ~ Petal.Length, 
         color = ~ Species,
         data = iris_sample,
         shape = ~Species,
         size = 4) %>% 
  gf_theme(legend.position = "none") %>% 
  gf_labs(title = "Geschichtete Zufallsstichprobe",
          subtitle = "Von jeder Farbe 10 zufällig ausgewählte Beobachtungen",
       x = "X",
       y = "Y")  +
  theme(axis.ticks = element_blank(),
        axis.text = element_blank(),
...
set.seed(42)
iris_sample <- slice(iris, size = 1:50)
iris_sample2 <- sample_n(iris_sample, size = 30)
...
p4 <- ggplot(iris) +
  ...
  geom_jitter(color = "grey80",
             size = 4,
             aes(shape = Species)) +
  geom_jitter(data = iris_sample2,
             aes(color =  Species,
                 shape = Species),
             size = 4) +
  theme(legend.position = "none") +
  labs(title = "Gelegenheitsstichprobe",
          subtitle = "Von den ersten 50 Beobachtungen 30 zufällig ausgewählte",
       x = "X",
       y = "Y") +
  theme(axis.ticks = element_blank(),
        axis.text = element_blank(),
...
| $\epsilon$| *epsilon*  | Symbol für Residuum
...
### Übersicht: Quantitativer Forschungsprozess {#quantFoprozess}

```{r forschungsprozess-image, echo=FALSE, out.width = "90%", fig.align="center"}
knitr::include_graphics(file.path(pathToImages, "Forschungsprozess.png"))
```
