...
- Die Studierenden kennen zentrale Operationen der Datenvorverarbeitung und können sie anwenden (wie Zeilen filtern).
- Die Studierenden können einen Datensatz von Lang- zur Breitform pivotieren und können Anwendungsbeispiele beschreiben.
- Die Studierenden können Unterschiede in den Ursachen von fehlenden Werten erläutern und kennen Strategien zum Umgang mit fehlenden Werten.
- Die Studierenden wissen um potenzielle Probeleme, die Ausreißerwerte verursachen und können sie im Datensatz identifizieren.
...
Häufig liegt ein Datensatz in *langer* Form vor, aber man benötigt ihn in *breiter* Form (oder umgekehrt):
...
### Datensatz in *Lang*form überführen mit `pivot_longer()`
...
### Datensatz in *Breit*form überführen mit `pivot_wider()`
...
Fehlende Werte (`NA`s)  willkürlich erzeugen im Datensatz `tips`:
...
Mit `inspect()` bekommt man die Ausgabe  von `favstats()` für jede nummerische Spalte des Datensatzes, und damit auch die Angabe der Anzahl fehlender Werte.
...
Cook's Distance $D_i$ vergleicht die vorhergesagten Werte  $\hat{y}_i$ für jeden Fall $i$ des Datensatzes mit der Vorhersage, wenn Beobachtung $i$ entfernt wurde ($y_{(i)}$). Diese Differenzen werden dann quadriert und aufsummiert. Der Nenner dient der Standardisierung.
...
So können Sie ggf. die einflussreichen Beobachtungen aus einem Datensatz entfernen:
