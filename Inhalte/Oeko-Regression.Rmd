...
[Wo können Sie dies Verfahren einsetzen?]{.cemph}
...
- [Überwachtes Lernen]{.cstrong} (engl.: *supervised learning*): Kann ein Teil der Variation einer abhängigen Variable $y$ durch unabhängige Variable(n) $x$ modelliert werden: $y = f(x) + \epsilon$^[$\epsilon$: (zufälliger) Fehler, Residuum]?
...
- [Methode der kleinsten Quadrate:]{.cstrong} Bestimme Vektor $\hat{\beta}=\begin{pmatrix}\hat{\beta}_0\\ \hat{\beta}_1\end{pmatrix}$ so, dass für $\hat{\epsilon}_i = y_i - \hat{f}(x_i)  = y_i - (\hat{\beta}_0 + \hat{\beta}_1 x_i)$ der Wert $\sum \hat{\epsilon}^2_i$ minimal ist.
...
- Dabei nehmen wir an, dass innerhalb der Maschnine ein linearer Zusammenhang vorliegt: unsere Maschine wird so konstruiert.^[Skizze: Sebastian Sauer]
...
$$\widehat{tip} = `r sprintf("%.4f", erglm1$coefficients[1])` + `r sprintf("%.4f", erglm1$coefficients[2])` \cdot \text{total\_bill}$$


### Übung `r nextExercise()`: Prognose der Trinkgeldhöhe aus Rechnungshöhe {.exercise type=yesno answer=no}

Für ein gegebenes $x_0 = 10$ lautet die Prognose $\hat{y}_0 = `r sprintf("%.4f", erglm1$coefficients[1])` + `r sprintf("%.4f", erglm1$coefficients[2])` \cdot 10 = `r round(erglm1$coefficients[1],4) +  round(erglm1$coefficients[2],4)*10`$.
...
::: {.notes}
***Nein***, nur wenn das Residuum, der Fehler $\epsilon=0$ ist. Im Mittelwert des Modells der Stichprobe liegt das Trinkgeld aber bei  $`r round((round(erglm1$coefficients[1],4) +  round(erglm1$coefficients[2],4)*10),2)`\,\$$.
:::


### Prognoseintervalle

Punktprognose inkl. $95\%$-Prognoseintervall:^[*Hinweis:* Das Prognoseintervall (`"prediction"`) ist breiter als das Konfidenzintervall (`"confidence"`): Während das erste sich auf die Unsicherheit bzgl. des Wertes $y$ gegeben $x$ bezieht, bezieht sich das zweite auf die Unsicherheit bzgl. des Mittelwertes von $y$ gegeben $x$.]
...
Ein Regressionsmodell liefert folgende Ausgabe: `Multiple R-squared = `r round(summary(erglm1)$r.squared,4)` `.]
...
```{r resample-use-print-original, echo=FALSE}
my_point(y ~ x,         
...
:::
::: {.column width="30%" .tiny}
```{r resample-use-print-resample-1, echo=FALSE}
...
:::
::: {.column width="30%" .tiny}
```{r resample-use-print-resample-2, echo=FALSE}
my_point(y ~ jitter(x), data=resample(df), caption="resample" )
```
...
- Resampling hat ergeben, dass für $95\%$ unser *Resampling*-Stichproben die geschätzte Steigung $\hat{\beta}^*_1$ zwischen `r floor(quantile( ~ total_bill, data = Bootvtlg, probs = c(0.025))*100)/100` und `r ceiling(quantile( ~ total_bill, data = Bootvtlg, probs = c(0.975))*100)/100` liegt.
...
`quantile( ~ total_bill, data = Bootvtlg,probs = c(0.005, 0.995))`:

`r quantile( ~ total_bill, data = Bootvtlg,probs = c(0.005, 0.995))`
...
Angenommen, es gelte $x$ und $y$ stehen in keinem (linearen) Zusammenhang.
...
::::::::: {.columns}
::: {.column width="30%" .tiny}
```{r sample-use-print-original, echo=FALSE}
...
:::
::: {.column width="30%" .tiny}
```{r sample-use-print-shuffle-1, echo=FALSE}
...
:::
::: {.column width="30%" .tiny}
```{r sample-use-print-shuffle-2, echo=FALSE}
my_point(y ~ shuffle(x), data=df, caption="shuffled" )
```
...
Wie schmeckt der Kaffee ($\hat{\beta}_0$), wenn die Größe der Bohnen ($x$) keine Rolle spielt ($\beta=0$)? Wir simulieren diese Maschine und gucken, ob der Kaffee, den wir probiert haben (Stichprobe, $\hat{\beta}$), so ähnlich schmeckt.^[Skizze: Sebastian Sauer]
...
::: {.notes}
Wenn die Nullhypothese gilt, liegen z. B. 95% der Werte im Bereich `r round(quantile( ~ total_bill, data = Nullvtlg, probs = c(0.025)),2)` bis `r round(quantile( ~ total_bill, data = Nullvtlg, probs = c(0.975)),2)`:

`quantile( ~ total_bill, data = Nullvtlg, probs = c(0.025, 0.975))`:

`r quantile( ~ total_bill, data = Nullvtlg, probs = c(0.025, 0.975))`
...
- $\frac{\hat{\beta}-0}{\widehat{se}}=\frac{`r sprintf("%.2f", erglm1$coefficients[2])`-0}{`r sprintf("%.2f", sd( ~ total_bill, data = Bootvtlg))`}=`r round(erglm1$coefficients[2]/sd( ~ total_bill, data = Bootvtlg),2)`$ Einheiten gemessen in Standardfehlern ist der beobachtete Wert der Stichprobe $\hat{\beta}_1$ vom angenommenen Wert $0$ entfernt.
...
Ist die Erfüllung der Annahme einer Normalverteilung für die Residuen hier fragwürdig?

- Ja.
- Nein.

::: {.notes}
***Ja*** -- die Verteilung der Residuen scheint zwar keine extreme Schiefe zu haben (siehe Histogramm), aber es gibt mehr extreme Werte (d. h. Werte in den Rändern der Verteilung) als bei einer Normalverteilung zu erwarten (siehe Q-Q-Plot, vgl. Literatur).
...
Wenn die Residuen identisch verteilt sind, sollte im **Residualplot**, d. h. z. B. im Streudiagramm der Residuen (`resid()`) gegen die angepassten Werte (`fitted()`), *kein* Muster zu erkennen sein.
...
::::::::: {.columns}
::: {.column width="49%"}
```{r gf-point-tip-smoker, echo=TRUE, eval=FALSE, fig.asp = 0.618}
...
:::
::: {.column width="49%"}
```{r ref.label="gf-point-tip-smoker", eval=TRUE, echo=FALSE, out.width = "95%", fig.align="center"}
```
:::
:::::::::
...
```{r, echo=FALSE}
contrasts(tips$smoker) %>% knitr::kable()
```
...
```{r lm-smoker-shuffle, out.width = "40%", fig.align="center", echo=TRUE}
set.seed(1896)
...
### Regression eines Anteils {exclude=bda}
...
Woran können Sie noch am ehesten in einem linearen Modell erkennen, ob Sie ein *gutes* Modell haben -- bei einer metrischen abhängigen Variable $y$?
...
::: {.notes}
*A* und *B* sind falsch, da der p-Wert nur aussagt, wie wahrscheinlich die Daten unter $H_0: \beta_i=0$ sind. *C* und *D* sind falsch, da $|\hat{\beta}_i|$ u.a. davon abhängt, in welcher Einheit $y$ oder $x$ gemessen werden (z.B. Gewicht in $g$ oder $kg$). $R^2$ gibt den Anteil der im Modell und Stichprobe modellierten Variation von $y$ an, daher ist ***E*** korrekt. 

Zusätzlich sollte es aber in einem brauchbaren Modell auch signifikante und von der Größe her relevante geschätzte Koeffizienten geben. Aber wann $R^2$ *groß* ist, hängt von der Varianz und Anwendung ab. Auch sagt $R^2$ nicht, ob das Modell stimmt (siehe z.B. auch Scheinkorrelation), und es ist nicht robust gegen Ausreißer.
:::
...
Interpretation der Koeffizienten (Schätzwerte, p-Werte): unter sonst gleichen Umständen, d. h., die anderen Variablen bleiben im Modell konstant/ unverändert (c. p.): marginaler Effekt.^[Durch Versuchsplanung oder eine vorgelagerte Hauptkomponentenanalyse können unabhängige erklärende Variablen $x_j$ erzeugt werden. Vgl. auch partielle Ableitungen.]
...
Können sich die geschätzten Werte ändern, wenn Variablen ins Modell hinzugenommen oder weggenommen werden?

- Ja.
- Nein.

::: {.notes}
***Ja*** -- insbesondere, wenn assoziierte Variablen mit ins Modell aufgenommen werden. Das Vorzeichen der Koeffizienten kann sich sogar ändern (vgl. Simpsons Paradox). 
:::
...
Gegeben die Rechnungshöhe: Ist der in der Stichprobe beobachtete Wert $\hat{\beta}_2=`r round(summary(erglm3)$coefficients[3,1],2)`$, wenn eigentlich gilt $\beta_2=\beta_{\text{smoker}}=0$, sehr ungewöhnlich?

- Ja.
- Nein.

::: {.notes}
***Nein***:  Die $0$ liegt im $95\%$-Bootstrap-Konfidenzintervall von `r round(confint(Bootvtlg)[3,2],2)` bis `r round(confint(Bootvtlg)[3,3],2)`. Der beobachtete lineare Zusammenhang ist also nicht unwahrscheinlich -- wenn wir annehmen, dass gar keiner vorliegt.
:::


### Modellierte und nicht modellierte Variation {exclude=bda}
...
### $F$ und Zufallsmodell {exclude=bda}
...
### Wechselwirkung, Interaktion {exclude=bda}
...
- Ist unser Ziel, die Maschnine zu verstehen: $\hat{f}$.
...
```{r child = './useR/useR-post-OLS.Rmd', eval = showuseR}
```
