...
::: {.notes}
***B*** ist korrekt, da die Säule für `Male` höher als der für `Female` ist. 
:::
...
C.  Die relative Häufigkeit einer Rechnungshöhe von höchstens $10\,\$$ liegt 
    bei liegt bei $`r round(pdata( ~total_bill, q=10, data = tips),2)`$.

::: {.notes}
Die (empirische) Verteilungsfunktion sagt aus, wie groß der Anteil der 
Beobachtungen ist, die $\leq x$ sind, also ***C***.
Damit liegt dir relative Häufigkeit einer Rechnungshöhe *über* $10\,\$$ bei 
...
- **Arithmetischer Mittelwert** (engl. mean)^[Darüberhinaus gibt es noch den geometrischen und den harmonischen Mittelwert.]: Summe aller Beobachtungswerte $x_i$ geteilt durch die Anzahl Beobachtungen $n$: $\bar{x}=\frac{1}{n}\sum_{i=1}^n x_i$;
...
Die gilt auch wenn nominale Daten numerisch, d.h. als Zahlen kodiert werden. 
...
### Mittelwert als "Modell" -- Was heißt das konkret? {include-only=norman-only}
...
`inspect()` *inspiziert* den Datensatz und gibt eine Übersicht über 
...
### Gruppenmittelwerte als "Modell" -- Was heißt das konkret? {include-only=norman-only}
...
Visualisierung für den reduzierten Datensatz mit nur 1 oder 6 Personen am 
...
# Datensatz auf die Tischgrößen 1 und 5 reduzieren. 
...
***Nein***, wie Sie auch an der Graphik erkennen können. 
Z.B. gibt es in der Graphik 4 des Anscombe-Quartett eigentlich gar keinen 
...
### Hypothesen und Datengenerierender Prozess
...
### Offene Übung `r nextExercise()`: Was wäre wenn ... {.exercise type=essay}
...
### Übung `r nextExercise()`: Vergleich Daten und Modell {.exercise type=yesno answer=no}
...
bei `r round(dbinom(8,8, prob=0.5),4)`.
...
- Der Datensatz Sarah_Raet soll sein:
...
```
"Ich weiß, dass Datenvisualisierung ein heißes Thema ist, aber gehst Du hier 
nicht ein wenig zu weit?"^[[https://www.CAUSEweb.org/](https://www.causeweb.org/cause/caption-contest/august/2016/results) &copy; J.B. Landers, Überschrift B. Osyk]
