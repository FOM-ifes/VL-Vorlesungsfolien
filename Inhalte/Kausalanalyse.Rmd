...
- kennen die "Atome" von Kausalstrukturen (Forks, Collider, Chains) und können diese anhand von Beispielen erläutern.
...
### Zusammenhang Schlafzimmer und Preis^[Für mehr Informationen über den Datensatz siehe `?SaratogaHouses`. Idee: [De Veaux (2019) *Data Science for All*](https://iase-web.org/conference/satellite19/docs/Data%20Science%20for%20All.pdf)]
...
- Angenommen innerhalb der einzelnen Länder ($C$) gibt es keinen Zusammenhang zwischen Essenspräferenz ($A$) und Sportart ($B$). 
D.h., z.B. in Deutschland ($C$) wissen Sie nicht, ob jemand, der Schweinshaxe isst ($A$), auch Fußball spielt ($B$). 
Unbedingt (d.h. weltweit) sind die Wahrscheinlichkeiten nicht unabhängig: In anderen Ländern gibt es andere Ess- und Sportgewohnheiten.  
...
- Wenn ich weiß, dass *Sonnenschein* ist, enhält die Information, dass viel *Eis* verkauft wurde, keine zusätzliche Information über *Sonnnencreme*. Bedingt *Sonnenschein* sind *Eis* und *Sonnencreme* unabhängig.
...
- Wenn ich weiß, dass ich ausschlafen kann, weiß ich, dass entweder *Wochenende* oder *Urlaub* (oder beides) ist. Bei *Ausschlafen* lerne ich aus der Information kein *Urlaub*, dass *Wochenende* ist. Bedingt *Ausschlafen* sind *Wochenende* und *Urlaub* nicht unabhängig.
- Graphisches Modell: Wochenende $\rightarrow$ Ausschafen $\leftarrow$ Urlaub.
...
pfc <- gf_point(y ~ x, col = ~ z, data = Fork) %>%
  ...
  scale_color_viridis_d() + theme(legend.title = element_blank()) +
...
### Beispiel: Marginale Unabhängigkeit, Bedingte^[Idee: [https://fabiandablander.com/r/Causal-Inference](https://fabiandablander.com/r/Causal-Inference)] {include-only=master}
...
| ---------------------------|---------------------------------|--------------------------------|--------------------------------|
| Name                       | Chain (Kette)                   | Fork (Gabel)                   | Collider (Kollision)           
...
- $A$ und $B$ haben eine gemeinsame Ursache: *C* 
...
- $A$ und $B$ haben eine gemeinsame Wirkung: *C* 
...
### Collider {include-only=deprecated}

Eine Kollision (Collider) liegt bei folgender Graphenstruktur vor: 

$$X \rightarrow C \leftarrow Y$$

### Übung `r nextExercise()`: Collider: Unbedingte Unabhängigkeit {.exercise type=yesno answer=yes include-only=deprecated}
...
### Übung `r nextExercise()`: Collider: Bedingte Unabhängigkeit {.exercise type=yesno answer=no include-only=deprecated}
...
- Ansatz: 
  ...
  - Erzeuge keinen Scheinpfad (Collider)
...
::: {.notes}
Der nicht-kausale Pfad $X \leftarrow C \rightarrow Y$ (Fork), der durch die *Hintertür* (engl. *Backdoor*) sollte blockiert werden, also ***Ja***.
