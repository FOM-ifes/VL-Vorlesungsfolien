...
Die geschätzten $y$-Werte werden mit $\hat y$ bezeichnet: $\hat y = \hat{\beta}_0 + \hat{\beta}_1 x_i$.
...
- Die Quadratsumme der Residuen ($\hat{\epsilon}$), $\sum \hat{\epsilon}^2_i$, hängt ab vom geschätzten Achsenabschnitt ($\hat{\beta}_0$) und der geschätzten Steigung ($\hat{\beta}_1$): Minimiere $\sum \hat{\epsilon}^2_i$ durch optimale Wahl von $\hat{\beta}_0$ und $\hat{\beta}_1$.
...
### Modellzusammenfassung {include-only=MasterNeu}
...
Koeffizienten des Modells `erglm1`:
...
Das Bestimmtheitsmaß sagt *nicht*, ob ein lineares Modell stimmt. 
Im Falle der *Anscombe*-Daten gilt in allen Fällen $R^2 \approx 0.67$:
...
### Schema: Bootstrap {include-only=MasterNeu}
...
### Übung `r nextExercise()`: Bootstrap {.exercise type=yesno answer=yes include-only=MasterNeu}
...
### Vergleich Sampling und Resampling {include-only=MasterNeu}
...
```{r include = FALSE}
DETpopulation <- rep(factor(c("f","r")), c(220000, 120000))
set.seed(1896) # Reproduzierbarkeit
DETStiprovtlg <- do(10000) * prop( ~ sample(DETpopulation, size = 34), success = "r")
DETstipro <- rep(factor(c("f","r")), c(22, 12))
set.seed(1896)
DETBootvtlg <- do(10000) * prop( ~ resample(DETstipro), success = "r")
```


```{r LR_BootCompare, echo=FALSE, fig.align="center", fig.width = 12, fig.height = 6, out.width="12cm"}
p1 <- gf_bar( ~ prop_r, data = DETStiprovtlg) %>% 
  gf_labs(x = "Anteil", title = "Stichprobenverteilung", subtitle = "10000 Stichproben aus einer \nPopulation")
p2 <- gf_bar( ~ prop_r, data = DETBootvtlg) %>% 
  gf_labs(x = "Anteil", title = "Bootstrap-Verteilung", subtitle = "10000 Resample-Stichproben aus \nder Originalstichprobe")

grid.arrange(grid.arrange(p1,p2, ncol=2))
```
...
### Hintergrund: Punktschätzung {include-only=MasterNeu}
...
- Oder wie hier in der Regression: Population $\beta$, Punktschätzer $\hat\beta$.

Das Symbol *Dach* ($\hat{}$) zeigt, dass der *unbekannte, wahre* Wert *geschätzt* wurde. Punktschätzer sind Funktionen der Stichprobe. Der konkrete Wert gilt nur für das Modell der Stichprobe.

<!-- -->

### Hintergrund: Standardfehler {include-only=MasterNeu}
...
### Hintergrund: Konfidenzintervall {include-only=MasterNeu}
...
- Häufig bei $n>30$: $95\,\%$-KI $\approx \delta^* \pm 2 \cdot se$.^[Vgl. auch die $68$-$95$-$99.7\,\%$-Regel bei Normalverteilung.]  
  So ergibt sich z.B. für den Mittelwert: $95\,\%$-KI für $\mu$: $\approx \bar{x} \pm 2 \cdot se = \bar{x} \pm 2 \cdot \frac{sd}{\sqrt{n}}.$

- Je größer der Stichprobenumfang, desto schmaler ist das Konfidenzintervall (unter sonst gleichen Umständen): Der Standardfehler $se$ fällt mit $n$.

- Je größer die Sicherheit (z.B. $99\,\%$ statt $95\,\%$), desto breiter ist das Intervall. 


### Hintergrund: Überdeckung durch Konfidenzintervalle {include-only=MasterNeu}

```{r Inf_KIs, echo=FALSE, , fig.align="center",  fig.width = 9, fig.asp = 2/3, out.width="12cm"}
set.seed(1896) # Reproduzierbarkeit
CIsim(n = 10, samples = 100)    
```


### FOMshiny: Sampling, Resampling und KI {include=MasterNeu}
...
- Wie verändert sich das Konfidenzintervall, wenn bestimmte Einflussgrößen geändert werden?
...
Wie schmeckt der Kaffee ($\hat{\beta}$), wenn die Größe der Bohnen ($x$) keine Rolle spielt ($\beta=0$)? Wir simulieren diese Maschine und gucken, ob der Kaffee, den wir probiert haben (Stichprobe, $\hat{\beta}$), so ähnlich schmeckt.^[Skizze: Sebastian Sauer]
...
### Teststatistik und p-Wert {include-only=MasterNeu}
...
### Schema der simulationsbasierten Inferenz {include-only=MasterNeu}
...
### Hintergrund: Nutzen und Grenzen des p-Werts {include-only=MasterNeu}
...
### Hintergrund: Verteilung p-Wert {include-only=MasterNeu}
...
```{r Inf_pVals, echo=FALSE, fig.width = 9, fig.asp = 2/3, out.width="10.5cm", fig.align="center"}
...
### Hintergrund: Alternativhypothese und Signifikanz {include-only=MasterNeu}
...
### Hintergrund: Hypothesen prüfen in Analogie zu Gerichtsverfahren {exclude=qmwinf,eufom include-only=MasterNeu}
...
### $H_0$ und $H_A$ in der Regression {include-only=MasterNeu}

- Die Nullhypothese in der Regression lautet $H_0:\beta_1=0$ -- es gibt keinen Zusammenhang.
- Die Alternative ist daher das Gegenteil $H_A: \beta_1\neq 0$ -- es gibt einen Zusammenhang.
- Hier handelt es sich um eine zweiseitige oder ungerichtete Alternativhypothese.

- Wenn der p-Wert sehr klein ist, spricht das gegen die Nullhypothese.
- Formal: p-Wert $< \alpha$, daraus folgt $H_0$ verwerfen, was für einen Zusammenhang spricht. Das Ergebnis ist signifikant zum Niveau $\alpha$.
- Gilt p-Wert $> \alpha$, wird die $H_0$ nicht verworfen, was nicht für einen Zusammenhang spricht. **Aber:** Das Ergebnis spricht trotzdem nicht für *keinen* Zusammenhang.


### Cartoon: Signifikanz {include-only=MasterNeu}
...
### Hintergrund: Fehlerarten (I/II) {include-only=MasterNeu}

Wir können uns irren, da wir ja nur eine Stichprobe gezogen haben.

Wenn wir die $H_0$ verwerfen, könnte es ja dennoch sein, dass sie in Wirklichkeit zutrifft: 

- Fehler 1. Art oder $\alpha$-Fehler

  Die maximale Wahrscheinlichkeit für einen solchen Fehler wird begrenzt durch das Signfikanzniveau $\alpha$.
  
Auch wenn wir die $H_0$ nicht verwerfen, kann es sein, dass in Wirklichkeit $H_A$ gilt:

- Fehler 2. Art oder $\beta$-Fehler

  Die Wahrscheinlichkeit dieses Fehlers ist schwieriger zu bestimmen^[Siehe z.B. Paket [pwr](https://cran.r-project.org/package=pwr).] und kann nicht im Vorhinein begrenzt werden. Bei guten Tests sinkt sie mit größerem Stichprobenumfang $n$.
...
### Hintergrund: Fehlerarten (II/II) {include-only=MasterNeu}

```{r Inf_Errors, echo=FALSE,  fig.width = 12, fig.asp = 2/3, out.width="12cm", fig.align="center"}
...
### FOMshiny: Stichprobenumfang {include=MasterNeu}
...
```{r echo=FALSE, out.width = "20%", fig.align="right"}
include_graphics(file.path(pathToImages, "maschine.jpg"))
```
...
### Hintergrund: Verteilungen {include-only=MasterNeu}

Allgemein sagt eine Verteilung, wie wahrscheinlich bzw. häufig Werte bzw. Wertebereiche sind (siehe auch Kapitel zu Wahrscheinlichkeitsverteilungen)
...
### Übung `r nextExercise()`: Verteilungen {.exercise type=A-B-C-D-E answer=B include-only=MasterNeu}
...
### Modellzusammenfassung: `summary`

In der Modellzusammenfassung werden neben den Schätzern für die Koeffizienten (Spalte `Estimate`) auch die p-Werte für die Koeffizienten (Spalte `Pr(>|t|)`) sowie das Bestimmtheitsmaß $R^2$ (`Multiple R-squared: ...`) ausgegeben.
...
```
"Punkte, die von ihren Peers abweichen, sind häufig die interessantesten."^[[https://www.CAUSEweb.org/](https://www.causeweb.org/cause/caption-contest/march/2018/results) &copy; J.B. Landers, Bildunterschrift J. Alloway]
...
*Annahme:* Fehler sind identisch verteilt. Überprüfung z.B. über Streudiagram $\hat{\epsilon}$ und $\hat{y}$.
...
### Regression: Trinkgeld auf Raucher/Nichtraucher {exclude=bda,MasterNeu}
...
### Regression: Trinkgeld auf Raucher/Nichtraucher {include-only=MasterNeu}
...
```{r lm-smoker-shuffle, out.width = "40%", fig.align="center", echo=FALSE}
set.seed(1896)
...
\end{pmatrix}$$
  ...
  - $\hat{\mathbf{\upbeta}}$ der Spaltenvektor der Koeffizienten: $\hat{\mathbf{\upbeta}}^\text{T}=\begin{pmatrix} \hat{\beta}_0, & \hat{\beta}_1, & \cdots, & \hat{\beta}_p \end{pmatrix}$

- Dann gilt:

$$\hat{\mathbf{\upbeta}}=(\textbf{X}^\text{T}\textbf{X})^{-1}\textbf{X}^\text{T}\textbf{y}$$
...
::: {.footnotesize}
Modelliere die Trinkgeldhöhe als lineare Funktion von Rechnungshöhe und Raucher/ Nichtraucher (hier mit Modellzusammenfassung `summary`):
...
plotModel(erglm3, col = pal_npg()(2)) %>% 
  gf_relabel(.color = "smoker") %>%
  gf_refine(scale_colour_npg())
```
...
$$\widehat{`r lm_y`}_i = `r beta[1]` `r plmi(beta[2])` \cdot `r lm_x[2]`_i `r plmi(beta[3])` \cdot \begin{cases} 1 &: smoker_i = \text{"Yes"} \\ 0 &: smoker_i = \text{"No"} \end{cases}$$

Dies lässt sich in *zwei* lineare Modellgleichungen umschreiben:

1. Die Modellgleichung für die Tische mit Rauchern:
    $$\begin{aligned}\widehat{`r lm_y`}_{i,smoker_i=\text{"Yes"}} &= `r beta[1]` `r plmi(beta[2])` \cdot `r lm_x[2]`_i `r plmi(beta[3])` \\ &= `r beta[1]` `r plmi(beta[3])` `r plmi(beta[2])` \cdot `r lm_x[2]`_i \\ &= `r beta[1]+beta[3]` `r plmi(beta[2])` \cdot `r lm_x[2]`_i\end{aligned}$$

2. Die Modellgleichung für die Nichtraucher-Tische:
    $$\widehat{`r lm_y`}_{i,smoker_i=\text{"No"}} = `r beta[1]` + `r beta[2]` \cdot `r lm_x[2]`_i$$
...
### Hintergrund: Hypothesenprüfung und Konfidenzintervall {include-only=MasterNeu}
...
Das beobachtete Verhältnis an erklärter zu nicht-erklärter Variation in der Stichprobe, $F=`r round(summary(erglm3)$fstatistic[1],2)`$, ist viel größer als die im (globalen) Nullmodell simulierten Werte. Das Nullmodell scheint ungeignet zur Erklärung der beobachteten Daten (vgl. `p-value`).
...
gf_lm(tip ~ total_bill , data = tips.s.ext) %>%
  ...
  gf_segment(tipd+tip0 ~ total_bill + total_bill, color = pal_npg()(2)[1], size = 2) %>%
  gf_segment(tipd+tip ~  total_bill + total_bill, color =  pal_npg()(2)[2], size = 2) %>%
...
## Regression mit der Zeit als unabhängige Variable {include-only=deprecated}

### Grundlagen Zeitreihenanalyse {include-only=deprecated}
...
### Analyse CO~2~-Konzentration {include-only=deprecated}
...
### CO~2~-Daten {include-only=deprecated}
...
### Übung `r nextExercise()`: CO~2~-Konzentration {.exercise type=yesno answer=yes include-only=deprecated}
...
### Regression CO~2~-Konzentration auf Zeit und Monat {include-only=deprecated}
...
### Übung `r nextExercise()`: Monatseffekte  {.exercise type=A-B-C answer=A include-only=deprecated}
...
### Hinweise {include-only=deprecated}
...
Grafisch^[Das Farbschema im Diagramm kann z.B. so angepasst werden: `plotModel(erglm4) + scale_color_npg()`, wobei das Paket `ggsci` benötigt wird.]:
...
```{r, echo=FALSE, fig.align="right", out.width="20%"}
plotModel(erglm4, col = pal_npg()(2)) %>% 
  gf_relabel(.color = "smoker") %>%
  gf_refine(scale_colour_npg())
```
...
<!-- ### Ergebnis: Wechselwirkung {.shrink exclude=bda,MasterNeu} -->

### Ergebnis: Wechselwirkung {exclude=MasterNeu}
...
### Ergebnis: Wechselwirkung (das Modell){exclude=exclude-crazy-norman,MasterNeu}
...
```
$$\begin{aligned}
\widehat{tip}_i &= `r beta[1]` `r plmi(beta[2])` \cdot `r lm_x[2]`_i `r plmi(beta[3])` \cdot \begin{cases} 1 &: smoker_i = \text{"Yes"} \\ 0 &: smoker_i = \text{"No"} \end{cases} `r plmi(beta[4])` \cdot `r lm_x[2]`_i \cdot \begin{cases} 1 &: smoker_i = \text{"Yes"} \\ 0 &: smoker_i = \text{"No"} \end{cases} \\
                &= `r beta[1]` `r plmi(beta[3])` \cdot \begin{cases} 1 &: smoker_i = \text{"Yes"} \\ 0 &: smoker_i = \text{"No"} \end{cases} +  \left(`r beta[2]` `r plmi(beta[4])` \cdot \begin{cases} 1 &: smoker_i = \text{"Yes"} \\ 0 &: smoker_i = \text{"No"} \end{cases} \right) \cdot `r lm_x[2]`_i
\end{aligned}$$
:::
::: {.small}
Das sind *zwei* lineare Modellgleichungen:

1. Modellgleichung für die Tische mit Rauchern:
    $$\widehat{tip}_{i, smoker_i = \text{"Yes"}} = `r beta[1]+ beta[3]` `r plmi(beta[2]+beta[4])` \cdot `r lm_x[2]`_i$$

2. Modellgleichung für die Nichtraucher-Tische:
    $$\widehat{tip}_{i, smoker_i = \text{"No"}} = `r beta[1]` `r plmi(beta[2])` \cdot `r lm_x[2]`_i$$
:::


### Ergebnis: Wechselwirkung {exclude=bda include-only=MasterNeu}
...
### Ergebnis: Wechselwirkung (das Modell){exclude=exclude-crazy-norman include-only=MasterNeu}

Aus den *Punktschätzungen* der Koeffizienten (`coef(erglm4)`) erhalten wir das folgende Modell:
...
<!-- ### Ergebnis: Wechselwirkung {exclude=bda} -->
...
Die Wahl der *wichtigen* Variablen im Modell ist nicht trivial. Dabei wird ein Kriterium wie z.B. AIC^[Akaike-Informationskriterium, siehe z.B. [https://otexts.org/fpp2/selecting-predictors.html](https://otexts.org/fpp2/selecting-predictors.html).] zur Modellevaluierung verwendet. Mögliche Herangehensweisen sind z.B. 
...
### Wiederholung: PPDAC {include-only=MasterNeu}
...
[C]{.cemph} 
: (Conclusion)
: Schlussfolgerungen, Interpretation, neue Ideen, Limitationen, Kommunikation.

:::
::: {.column width="20%"}

```{r ppdac, echo=FALSE, out.width = "95%", fig.align="right"}
...
### Ablauf: Hypothesenprüfung {include-only=MasterNeu}
...
### ATOM^[Quelle: Wasserstein et al. (2019) Moving to a World Beyond "p < 0.05", The American Statistician, Vol. 73, Supplement 1, S. 1--19, [https://doi.org/10.1080/00031305.2019.1583913](https://doi.org/10.1080/00031305.2019.1583913)] {include-only=MasterNeu}
...
### Duhem-Quine-These und BENT {include-only=MasterNeu}
...
### Bayes-Statistik {include-only=MasterNeu}
