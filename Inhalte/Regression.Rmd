...
- Eine Kurvendiskussion^[über notwendige und hinreichende Bedingungen für die Existenz von Extremwerten] der Funktion $g(\hat{\beta}_0, \hat{\beta}_1)=\sum_{i=1}^n \left( y_i - (\hat{\beta}_0 + \hat{\beta}_1 x_i) \right)^2$ führt zu folgendem Ergebnis:
  ...
  - $\hat{\beta}_1=\frac{s_{x,y}}{s^2_x}=\frac{s_y}{s_x}r_{x,y}$, d.h., die geschätzte Steigung ist der Quotient aus der Kovarianz von $x$ und $y$ und der Varianz von $x$.
...
Punktprognose für $x_0=10$ mit Hife der Funktion `predict():`
...
*Annahme:* Fehler sind identisch verteilt.
...
A.  Die Varianz der Fehler scheint unabhängig von der Höhe der angepassten Werte zu sein.
B.  Die Varianz der Fehler scheint mit der Höhe der angepassten Werte zu steigen.
C.  Die Varianz der Fehler scheint mit der Höhe der angepassten Werte zu fallen.
...
Mittelwerte, $\bar{x}_{\text{no}}, \bar{x}_{\text{yes}}$:
...
Mittelwertsdifferenzen, $\bar{x}_{\text{yes}}-\bar{x}_{\text{no}}$:
...
::::::::: {.columns}
::: {.column width="49%"}
```{r gf-point-tip-smoker, echo=TRUE, eval=FALSE, fig.asp = 0.5}
...
Gegeben die Rechnungshöhe: Ist der in der Stichprobe beobachtete Wert $\hat{\beta}_2=`r round(summary(erglm3)$coefficients[3,1],2)`$ sehr unplausibel, wenn eigentlich die Annahme $\beta_2=\beta_{\text{smoker}}=0$ gilt?
...
Das beobachtete Verhältnis an erkärter zu nicht-erklärter Variation in der Stichprobe, $F=`r round(summary(erglm3)$fstatistic[1],2)`$, ist viel größer als die im (globalen) Nullmodell simulierten Werte. Dieses Modell scheint ungeignet zur Erklärung der beobachteten Daten (vgl. `p-value`).
...
Da sich in diesem Beispiel das Einflussgewicht von `total_bill` zwischen den beiden Gruppen `SmokerNo` und `SmokerYes` unterscheidet, spricht man von einer *Wechselwirkung*. Im Diagramm erkennt man die Wechselwirkung daran, dass die Regressionsgeraden der beiden Gruppen nicht parallel sind.
...
***Ja***: Anahnd der geschätzten Koeffizienten $\beta$ können die marginalen Zusammenhänge der unabhängigen Variablen $x$ auf die abhängige Variable $y$ nachvollzogen werden -- anders als z.B. bei künstlichen neuronalen Netzen, wo dies teilweise sehr schwer ist. 
