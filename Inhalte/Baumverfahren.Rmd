...
```
# `r nextChapter()` Baum-basierte Verfahren
...
```{r, echo=FALSE, fig.align="center", out.width="70%"}
b1b <- rpart(smoker~total_bill+tip, data=tips, minsplit = 30)
rpart.plot(b1b)
```
...
klaR::partimat(smoker~tip+total_bill, data=tips, method="rpart", minsplit=30) 
```
...
- Evtl. spiegeln sie menschliches Verhalten wieder
