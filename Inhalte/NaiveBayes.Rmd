...
set.seed(1896)
n <- 1000
daten <- data.frame(Geschlecht=rep(c("Frau", "Mann"), each=n), Groesse=c(rnorm(n, 165,7), rnorm(n, 178,7)))
nb_geschlecht <- NaiveBayes(Geschlecht~Groesse, data=daten)
plot(nb_geschlecht, lwd=2, main="Größenverteilung", col=c("red", "blue"), cex=1.5, lty = c(1,1))
...
Es gibt mehrere Implementierungen von Naive Bayes in R. Eine flexible Implementierung bietet das Paket `klaR`.
...
```{r}
progNB <- predict(ergNB, newdata = test)
```

*a posteriori* Wahrscheinlichkeiten:
```{r}
head(progNB$posterior)
```
...
```{r}
tally(test$sex ~ progNB$class)
```
```{r, echo=FALSE}
tt <- tally(test$sex ~ progNB$class)
```
...
Was ist die beste Interpretation der a posteriori Wahrscheinlichkeit von `r progNB$posterior[1,1]` für `r colnames(progNB$posterior)[1]`?

A.  Die Rechnungszahler\*in ist zu `r progNB$posterior[1,1]` `r colnames(progNB$posterior)[1]`.
B.  Die Rechnungszahler\*in ist zu `r 1-progNB$posterior[1,1]` `r colnames(progNB$posterior)[1]`.
C.  Bei der Rechnungshöhe und der Tagezeit können wir auf lange Sicht erwarten, dass `r progNB$posterior[1,1]` der Rechnungen von `r colnames(progNB$posterior)[1]` bezahlt werden.


::: {.notes}
Die Rechnungszahler\*in ist entweder eine Frau oder nicht. Wir können nur sagen, dass auf lange Sicht bei gegebenen $x$ der Anteil von $y=`r colnames(progNB$posterior)[1]`$ bei `r progNB$posterior[1,1]` in unserem Modell liegt (***C***).
...
`ergNBueb <- NaiveBayes(smoker ~ tip + size + time, data = tips)`  
`progNBueb <- predict(ergNBueb, newdata = test)`    
`mean( ~ (progNBueb$class != test$smoker))` 

Die Fehlklassifikationsrate liegt bei `r mean( ~ (progNBueb$class != test$smoker))`.
