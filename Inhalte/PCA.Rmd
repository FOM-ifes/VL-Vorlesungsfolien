...
Zusammenfassung der Multi-Item Likert Skala zu: *Schwierigkeit von Statistik*:^[Auf einer Skala von 1 (Trifft überhaupt nicht zu) über 4 (Weder zutreffend, noch unzutreffend) bis 7 (Trifft voll und ganz zu) [Candace Schau: Survey of Attitudes Toward Statistics, SATS-36](http://www.evaluationandstatistics.com/)]
...
**Cronbachs Alpha** ist eine Maßzahl für die **interne Konsistenz** einer Skala (*Reliabilität*). Sollte i.d.R. $>0.7$ sein.^[Wird i.d.R. für sog. Multi-Item Skalen verwendet. Hier eigentlich nicht angebracht, da keine Multi-Item Skala vorliegt!]

```{r echo=TRUE, eval=FALSE}
library(psych) # ggfs. einmalig vorab installieren
ca <- alpha(tipscale, check.keys = TRUE)
summary(ca)
```
...
library(psych) # ggfs. einmalig vorab installieren
ca <- psych::alpha(tipscale, check.keys = TRUE)
summary(ca)
```
:::
...
pcaUeb <- prcomp( ~. ,  data=tipscaleUeb)
summary(pcaUeb)
```
...
`tipscaleUeb <- tips %>% select(size, total_bill) %>% scale() %>% data.frame()`  
`pcaUeb <- prcomp( ~. ,  data=tipscaleUeb)` 
`summary(pcaUeb)`
</div>
