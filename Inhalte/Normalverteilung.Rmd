...
::: {.notes}
Richtig ist ***B***, für $x \rightarrow \infty$ gilt $F(x) \rightarrow 1$. Wenn $x$ immer größer wird, geht die Wahrscheinlichkeit für eine Beobachtung $\leq x$ gegen Eins. Für $x \rightarrow -\infty$ gilt $F(x) \rightarrow 0$. 
:::
...
*Hinweis:* Die Funktion `zscore()` verwendet statt der theoretischen Werte $\mu$ und $\sigma$ die beobachteten Werte $\bar{x}$ bzw. $sd$.^[Studentisierung]
...
### Übung  `r nextExercise()`: Verteilungsfunktion (II/II) {.exercise type=A-B-C-D answer=C}
...
Man gehört zu den $10\%$ besten, d.h. $90\%$ sind *nicht* besser, ab `r round(qnorm(0.9, mean = 55, sd = 15))` Punkten:
$$q=F^{-1}(p)$$
...
- Die Verteilungsfunktion $F(q)=p$, z.B. `xpnorm()`, gibt für einen gegebenen Wert `q` aus, mit welcher Wahrscheinlichkeit `p` dieser Wert nicht überschritten wird.

- Die Quantilsfunktion $F^{-1}(p)=q$, z.B. `xqnorm()`, gibt für eine gegebene Wahrscheinlichkeit `p` aus, welcher Wert `q` mit dieser Wahrscheinlichkeit nicht überschritten wird. 
...
Wie stark ändert sich der Ausschuss, wenn durch Qualitätssicherungsmaßnahmen sich die Streuung in der Produktion ändert? 
...
Ein QQ-Plot (`gf_qq()`) vergleicht die Quantile einer Verteilung z. B. mit den theoretischen einer Normalverteilung. Bei einer guten Übereinstimmung liegen die Punkte auf der Diagonalen.^[Formale Überprüfung z.B. über Shapiro-Wilk Test (`shapiro.test()`).]
