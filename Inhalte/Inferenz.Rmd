...
plot(c(1,2,3), y=c(1,sqrt(5),1), 
     ...
     pch=19, cex=8, ann=FALSE, axes=FALSE, xlim = c(0,4), ylim = c(0,3), asp = 1)
```
...
In der Regel kennen wir die Population nicht^[Genau genommen haben wir sie hier ja auch nur *simuliert*.]. Wir können aber unsere Stichprobe **resampeln** -- durch *Ziehen mit Zurücklegen*:
...
- Das $95\,\%$-Konfidenzintervall ist der Bereich in dem $95\,\%$ unserer wiederholten Stichprobenkennzahlen liegen:
...
::: {.notes}
***B***: Ein Fehler 2. Art liegt vor, wenn in Wirklichkeit $H_A$ gilt, die Testentscheidung aber lautet, dass $H_0$ nicht verworfen wird. I. d. R. kennen wir die Realität nicht, daher wissen wir nicht, ob ein solcher Fehler vorliegt. *Vorab* können wir aber durch eine Poweranalyse (siehe Literatur) die Wahrscheinlichkeit eines solchen Fehlers unter der Annahme eines gewissen Effekts (z. B. $\pi=\frac{2}{3}$) bestimmen und den nötigen Stichprobenumfang $n$ berechnen.
:::


### Wiederholung: Quantitativer Forschungsprozess

```{r ref.label="forschungsprozess-image", echo=FALSE, out.width = "90%", fig.align="center"}
```
...
### Übung `r nextExercise()`: Konfidenzintervall und Hypothesentest{.exercise type=yesno answer=yes}
