...
### Übung `r nextExercise()`: Forschungsprozess {.exercise type=essay}
...
Was können Sie über die Verteilung des Immobilienpreises aussagen?
```{r, fig.align="center", out.width="40%"}
gf_histogram( ~ Preis, data = Houses)
```

::: {.notes}
Der Preis ist unimodal rechtsschief verteilt. Der Schwerpunkt der Verteilung liegt ca. zwischen $150000\$$ und $200000\$$. Die Spannweite ca. zwischen $0\$$ und $800000\$$.
:::
...
Der Mittelwert der Stichprobe des Preises liegt bei $\bar{x}=`r mean(~ Preis, data = Houses)`$. Mit welchem Simulationsverfahren können Sie einen *plausiblen* Wertebereich für den unbekannten Wert der Population $\mu$ konstruieren?
...
Stimmt die Aussage: $95\%$ der Immobilien erzielen einen Preis zwischen $`r quantile(~mean, data = Bootvtlg, probs = c(0.025))`$ und $`r quantile(~mean, data = Bootvtlg, probs = c(0.975))`$ Dollar?
...
Stimmt die Aussage: Die Wahrscheinlichkeit das $H_0$ stimmt, liegt bei `r prop( ~ (mean <= 211966.7), data = Nullvtlg)`?

- Ja.
- Nein.

::: {.notes}
***Nein***: Wenn das Modell der Nullhypothese $\mu_0=215000$ stimmt, liegt die Wahrscheinlichkeit einen Mittelwert von $\bar{x}=211966.7$ (oder noch kleiner) zu beobachten, bei `r prop( ~ (mean <= 211966.7), data = Nullvtlg)`.
...
Wie oft liegt bei einem Signifikanzniveau von $\alpha=0.05$ der p-Wert unter $5\%$, wenn die Nullhypothese stimmt?
...
::: {.notes}
Gibt es einen Unterschied im mittleren Immobilienpreis für Immobilien mit oder ohne Kamin? Die Nullhypothese lautet $H_0: \mu_{\text{Kamin Ja}}=\mu_{\text{Kamin Nein}}$ gegen $H_A:  \mu_{\text{Kamin Ja}} \neq \mu_{\text{Kamin Nein}}$. Die Punktschätzer lauten $\hat{\mu}_{\text{Kamin Ja}}=`r mean(Preis ~ Kamin, data = Houses)[1]`$ bzw. $\hat{\mu}_{\text{Kamin Nein}}=`r mean(Preis ~ Kamin, data = Houses)[2]`$. In der Stichprobe gibt es also eine Differenz von $\bar{x}_{\text{Kamin Nein}}-\bar{x}_{\text{Kamin Ja}}$ von $`r meandiff_est`$.

In keiner der $10000$ Simulationen unter $H_0$ kam eine mindestens so große Abweichung vor. Mit einem p-Wert $<1/10000$ wird $H_0$ damit verworfen.
:::


### Übung  `r nextExercise()`: Schlussfolgerung {.exercise type=yesno answer=no}

Zeigt die Aussage, dass ein Kamin im Mittelwert zu einem um $`r abs(meandiff_est)`$ höheren Preis führt?
...
::: {.notes}
***Nein***: Der p-Wert (`Pr(>|t|)`) für $H_0: \beta_{\text{Kamin}}=0$ ist mit $`r round(coef(summary(erglm))[3,4],4)`$ größer als $\alpha=5\%$, $H_0$ wird nicht verworfen. Der Unterschied, gegeben die Wohnfläche, liegt auch nur noch bei `r round(coef(erglm)[3],2)` (statt $`r sprintf("%.2f", meandiff_est)`$ ohne Berücksichtigung der Wohnfläche).
:::
...
B.  `lm(Preis ~ Wohnflaeche + Kamin, data = Houses)`
...
```{r}
Houses <- Houses %>%
  mutate(Klimaanlage = relevel(Klimaanlage,
                               ref="Nein"))
...
### Übung  `r nextExercise()`: Datenzusammenfassung {.exercise type=A-B answer=A}

Mit welcher Methode lassen sich Variablen linear zusammenfassen?

A.  Hauptkomponentenanalyse 
B.  k-means Clusteranalyse

::: {.notes}
Während die Clusteranalyse i.d.R. Beobachtungen zusammenfasst bzw. Repräsentanten (Zentren) bestimmt werden, werden in einer PCA (***A***) numerische Variablen linear zusammengefasst.
:::


<!-- ### Übung  `r nextExercise()`: Baumverfahren {.exercise type=A-B answer=B} -->

<!-- Die Immobilienmaklerin möchte anhand des Alters der Immobilie modellieren welche Heizungsart vorliegt. Welches Verfahren ist geeignet? -->

<!-- A.  Regressionsbaum -->
<!-- B.  Klassifikationsbaum -->

<!-- ::: {.notes} -->
<!-- Da Heizung eine nominale Variable ist: ***B*** -->
<!-- ::: -->
